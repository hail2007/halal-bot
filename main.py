...
def duel_command(message):
    ...
    if len(args) != 2:
        bot.reply_to(message, "❗ /duel [сумма] (ответом на сообщение)\nПример: ответьте на сообщение и напишите /duel 500")
        return
    ...
    # Проверка, указан ли соперник через реплай
    if not message.reply_to_message:
        bot.reply_to(message, "❗ Ответьте на сообщение соперника!\nПример: ответьте на сообщение пользователя и напишите /duel 500")
        return
    ...
    # Отправляем инлайн-кнопки в чат, где вызвали
    ...
    sent_msg = bot.send_message(
        ...
        f"{challenger_name} вызывает {get_user_full_name(opponent_id)} на дуэль!\n"
    ...
    duel_requests[user_id]["message_id"] = sent_msg.message_id
    
    # Уведомляем вызывающего
    bot.reply_to(message, f"✅ Вызов отправлен!\nОжидайте ответа от {get_user_full_name(opponent_id)} в этом чате")
...
def handle_duel_response(call):
    ...
    if call.data.startswith('duel_accept_'):
        ...
        # Результат дуэли (отправляем в тот же чат)
        result_text = (
        ...
        if challenger_id in duel_requests:
            ...
            # Уведомляем вызывающего в том же чате
            bot.send_message(
                call.message.chat.id,
                f"❌ {get_user_full_name(call.from_user.id)} отклонил(а) вызов на дуэль!"
            )
...
def dep(message):
    ...
    # Проверка режима неуязвимости для админа (без надписи "режим бога")
...
dev_console_active = {}
give_cooldown = {}
duel_requests = {}  # {challenger_id: {"opponent_id": int, "amount": int, "timestamp": float, "chat_id": int, "message_id": int}}
db_lock = threading.Lock()
