...
def set_bot_commands():
    commands = [
        ...
        BotCommand("fixnames", "🔧 Обновить имена всех игроков (только админ)"),
    ]
    try:
...
def fixnames_command(message):
    if not dev_console_active.get(message.from_user.id, False) and message.from_user.id != ADMIN_ID:
        # Если консоль не активна, но команду вводит админ - всё равно работаем
        if message.from_user.id != ADMIN_ID:
            return
    
    bot.reply_to(message, "🔄 Обновляю имена всех пользователей... Это может занять время.")
...
def help_command(message):
    bot.reply_to(message,
        ...
        "🔧 `/fixnames` - обновить имена всех игроков (только админ)\n\n"
        "📱 **Кнопки** внизу экрана для быстрого доступа!",
        parse_mode="Markdown")

# --- ОТДЕЛЬНАЯ КОМАНДА /fixnames (работает без консоли) ---
@bot.message_handler(commands=['fixnames'])
def fixnames_standalone(message):
    if message.from_user.id != ADMIN_ID:
        bot.reply_to(message, "❌ Нет доступа! Эта команда только для админа.")
        return
...
if __name__ == '__main__':
    ...
    print("🔧 Команда для обновления имён: /fixnames")
